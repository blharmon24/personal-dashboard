...
- `ksp_missions`: `program_id` (FK cascade), `vessel_name`, `vessel_id` (FK → `ksp_vessels`, on delete set null — links a mission to a fleet vessel), `objective`, `deadline` (date), `assigned_crew` (text), `status` (proposed/active/complete/failed), `payout`, `bonus`, `penalty`, `briefing`, `completed_at`.
...
- `ksp_vessels` (Fleet, added v2026.06.17.3): `program_id` (FK cascade), `name`, `type` (Rocket/Lander/Spaceplane/Station/Rover/Probe/Satellite/Capsule/Other), `status` (planned/active/recovered/destroyed), `location` (free text — where it is now), `notes`, `created_at`. One vessel can fly many missions (reusable boosters, stations, rovers).
- All five: RLS `for all to authenticated using(true)`. Edge Function uses service role (bypasses RLS).

**Edge Function `ksp-president` (Deno; reference copy `ksp-president.ts` in repo, source of truth in Supabase dashboard):** Proxies Claude API (`claude-sonnet-4-6`, adaptive thinking — chosen for cost; was opus-4-8) so the API key stays server-side. **Requires secret `ANTHROPIC_API_KEY`** (Brian must set it on the function). Two actions: `new_president` (generates a randomized persona via structured output — name/title/persona/opening_message — and writes it to the program; `succession:true` for retirement) and `chat` (runs an agentic tool loop with tools `propose_mission`, `update_mission`, `set_salary`, `recommend_tech`, `update_tech`, `register_vessel`, `update_vessel`, executing DB writes via service key, returns `{reply, changed}`). The function strips the leading assistant greeting so the Anthropic API's "first message must be user" rule holds. `propose_mission`/`update_mission` accept an optional `vessel_id` to link the mission to a fleet vessel; the President is told to `register_vessel` a brand-new craft (then pass its id) or reuse an existing vessel's id, and to `update_vessel` status/location as ships fly. System prompt includes a "Current fleet (vessels)" section (vessels loaded server-side in `handleChat`).

**Client architecture:** Setup screen (name program → create row → invoke `new_president`). Stats bar (Earnings, Salary/wk, Active, Overdue, Crew, **Fleet** = count of `active`-status vessels). Tabs: 💬 President (chat window — Enter sends, persists `chat` to program, reloads data when `changed`, sends `tech_context` AND `fleet_context` so the President knows what's unlocked and what's flying), 🎯 Missions (grouped proposed/active/complete/failed; Accept→active, Decline→delete, Mark Complete→sets completed_at + applies bonus/penalty to balance, Mark Failed→no pay; deadline countdown badges), 🚀 Fleet (see below), 🌳 Tech Tree (visual — see below), 📋 R&D Plan (President's recommended unlock order from `ksp_tech` by `priority`, manual add/edit/delete), 👨‍🚀 Crew (roster CRUD), 🛰️ Capability (editable profile form).

**President rename (added v2026.06.17.4):** ✏️ Rename button in the `.pres-header` (President tab) → `renamePresident()` uses a `prompt()`, updates `president_name` on the program row, re-renders. Changes the **display name only** — `president_persona` (personality) is untouched. To re-roll personality, use 🔄 New President (succession). **Name generation** (`generatePresident` in `ksp-president.ts`) was upgraded to push creative, personality-telegraphing Kerbal names — pun/staff-riff examples (Thrustin, Mortlock, Wernsley, Kessler, Max-Q, etc.) baked into the persona prompt. Brian renamed his current President to **Kessler Kerman** via the new button.

**🚀 Fleet tab (added v2026.06.17.3):** Card grid of all vessels, sorted active→planned→recovered→destroyed. Each card shows type icon + name, type · 📍 location, a status badge (planned/In Flight/Recovered/Destroyed via `VESSEL_STATUS` map), "N flown" count, notes, and an inline mission history list (`missionsForVessel(v)` matches `m.vessel_id === v.id`, or legacy fallback to case-insensitive `vessel_name` match). Manual add/edit/delete via `modal-vessel` (`openVessel`/`saveVessel`/`deleteVessel`). Delete keeps the missions (FK `on delete set null`). `fleetContextString()` builds the fleet summary sent to the President. `VESSEL_TYPE_ICON` and `VESSEL_STATUS` maps near the top of the FLEET JS section.

**🌳 Tech Tree tab (visual, added v2026.06.17.2):** Faithful left-to-right node map, per Brian's spec ("flows left to right with arrows pointing to the next level from the feeder level"). Node datasets live in the HTML as `TREES = { stock: { name, nodes }, ctt: { name, nodes } }`; `STOCK_TREE` is the authentic stock KSP tree (64 nodes). `CTT_TREE` is the full Community Tech Tree (stock 62 nodes + ~82 CTT-specific nodes = ~144 total; sourced directly from `GameData/CommunityTechTree/Tree/CommunityTechTree.cfg` + `en-us.cfg` localization). The CTT includes all stock node IDs so switching trees preserves existing `tech_done` progress. A `<select id="tree-switcher">` dropdown in the tree tab header calls `switchTree(key)` which updates `program.tech_tree` in Supabase and re-renders. Each career picks its tree at setup (`tech_tree` col). Layout: columns = longest-path dependency depth from Start, rows minimized for crossings; SVG cubic-bezier arrows parent→child with a `context-stroke` arrowhead marker; absolutely-positioned node divs. Click a node to toggle completed → persists to `program.tech_done` and feeds the President via `techContextString()`. Completed nodes glow green; the President's recommended nodes (matched from `ksp_tech` by title/id) get a ★ ring. `getLayout()` caches by `tech_tree` key (layout is structural, independent of completion). Header buttons: "🔄 New President" (succession) and "🗑 Start Over" (delete + new game). Earnings = `banked` + live salary accrual (weeks since `salary_anchor_date` × base_salary) + sum of completed-mission net.

**Setup status — LIVE as of 2026-06-17:** tables created, `ksp-president` Edge Function deployed (model set to `claude-sonnet-4-6`), `ANTHROPIC_API_KEY` secret set, Brian loaded $5 of Anthropic API credit (pay-as-you-go, not a subscription). Confirmed working end-to-end (President greets on founding a program). Anthropic API cost was an unplanned expense for Brian — he chose Sonnet 4.6 over Opus 4.8 to stretch the credit; flag cost implications if suggesting a model bump.

**Open follow-ups (awaiting Brian's playtest feedback — he signed off 2026-06-17 right after first success):**
- Tech tree vertical layout is **depth-based (longest path from Start)**, not pixel-matched to the real in-game node positions. Offered to hand-tune row positions once Brian sees it live — he may want this.
- Wants his read on: President behavior (is he scaling missions to capability, naming vessels, negotiating, personality coming through?) and mission/pay balance.
- Likely tuning candidates next session: President tone, mission difficulty/pay balance, tech-tree layout. Adding more tech trees (Community Tech Tree) is a planned future option (dataset slots already structured in `TREES`).
- **Edge Function is deployed manually in Supabase** — pushing `ksp-president.ts` to GitHub does NOT update the live function. Any function change requires Brian to re-paste/redeploy in the Supabase dashboard (e.g. the model swap was done by editing the `MODEL` const there).

**Fleet feature — DEPLOYED & LIVE as of 2026-06-17 (v2026.06.17.4):** Brian ran the updated `ksp-setup.sql` (so `ksp_vessels` + the `vessel_id` column + RLS exist) and re-pasted `ksp-president.ts` into the `ksp-president` Edge Function in the Supabase dashboard. That single function redeploy also activated the richer Kerbal name generator (v2026.06.17.4). Commit `0f91622` on `main`. Both the Fleet (manual CRUD + President-managed vessels) and the improved name generation are fully live. Brian was going to rename his current President to "Kessler Kerman" via the new ✏️ Rename button.

NOTE for future sessions: the `ksp-setup.sql` is idempotent (`create table if not exists` / `add column if not exists` / `drop policy if exists`) — safe to re-run the whole file any time the schema changes.

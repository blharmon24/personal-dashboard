...
**⚠ PENDING DEPLOY for Fleet feature (v2026.06.17.3, built 2026-06-17):** Two manual steps before the Fleet works end-to-end: (1) Run the updated `ksp-setup.sql` in the Supabase SQL Editor — it's idempotent (`create table if not exists` / `add column if not exists` / `drop policy if exists`), so re-running the whole file is safe and just adds `ksp_vessels` + the `vessel_id` column + RLS. (2) Re-paste the updated `ksp-president.ts` into the `ksp-president` Edge Function in the Supabase dashboard so the President gets the `register_vessel`/`update_vessel` tools + fleet context. The client (Fleet tab, manual CRUD) works after step 1 alone; step 2 is only needed for President-managed vessels.
